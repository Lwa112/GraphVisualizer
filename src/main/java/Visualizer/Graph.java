    ...
    public void mouseClicked(MouseEvent e) {
        if (MainFrame.mode == Mode.ADD_A_VERTEX) {
            ...
            if (vertex != null) {
                ...
                // Run the algorithm (this also records the visit order internally)
                String path = algorithmSetter.execute(graph, vertex);
                List<Vertex> visitOrder = algorithmSetter.getVisitOrder();
                ...
                // Animate the visit order, one vertex per tick, then show the result
                animateVisitOrder(visitOrder, path);
            }
        }
    }

    private void animateVisitOrder(List<Vertex> visitOrder, String finalText) {
        ...
        timer.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent arg0) {
                if (index < visitOrder.size()) {
                    visitOrder.get(index).setHighlighted(true);
                    index++;
                } else {
                    // Animation finished - show the final result text
                    MainFrame.getAlgorithmDisplayLabel().setText(finalText);
                    timer.stop();
                }
            }
        });
